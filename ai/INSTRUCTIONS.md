...
This scraper extracts job listings from Talent Matchmakers careers page (Romania only) and imports them to peviitor.ro.
...
> **Important**: Scraper does NOT delete jobs from other sources (ANOFM, etc). It only upserts Talent Matchmakers jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from Talent Matchmakers website (Romania only)
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape Talent Matchmakers website (jobs for Romania)
    │
    ▼
...
| `tests/validate-talent-matchmakers-jobs.js` | CI fast validator (HEAD only); thin CLI over `src/job-validator.js` + `solr.js` |
...
| `tests/e2e/scraper.test.js` | End-to-end tests with real Talent Matchmakers website |
