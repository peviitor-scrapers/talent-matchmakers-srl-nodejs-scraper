...
describe('solr.js', () => {
  ...
  describe('querySOLR', () => {
    ...
    it('should throw when SOLR_AUTH is missing', async () => {
      delete process.env.SOLR_AUTH;
      await expect(solr.querySOLR('38460545')).rejects.toThrow('SOLR_AUTH not set in environment');
      process.env.SOLR_AUTH = 'test:test';
    });
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(solr.querySOLR('38460545')).rejects.toThrow('SOLR query error: 500');
    });
  });
  ...
  describe('queryCompanySOLR', () => {
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(solr.queryCompanySOLR('id:38460545')).rejects.toThrow('SOLR company query error: 401');
    });
  });
  ...
  describe('deleteJobsByCIF', () => {
    it('should delete all jobs for a CIF', async () => {
      ...
      await expect(solr.deleteJobsByCIF('38460545')).resolves.not.toThrow();
    });

    it('should throw on HTTP error', async () => {
      mockFetch.mockResolvedValue(makeErrorResponse(500, 'Error'));

      await expect(solr.deleteJobsByCIF('38460545')).rejects.toThrow('SOLR delete error: 500');
    });
  });
