...
describe('api.js', () => {
  ...
  describe('querySOLR', () => {
    it('should return normalized response with numFound and docs', async () => {
      mockFetch.mockResolvedValue(makeApiJobsResponse(2, [
        { id: 'job1', url: 'https://test.com/1', cif: '38460545' },
        { id: 'job2', url: 'https://test.com/2', cif: '38460545' }
      ]));

      const result = await api.querySOLR('38460545');
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(api.querySOLR('38460545')).rejects.toThrow('API jobs query error: 500');
    });

    it('should call API endpoint with correct URL', async () => {
      mockFetch.mockResolvedValue(makeApiJobsResponse(0, []));

      await api.querySOLR('38460545');

      expect(mockFetch).toHaveBeenCalledWith(
        'https://api.peviitor.ro/v1/scraper/jobs/?cif=38460545&rows=500',
  ...
  describe('getCompanyByCif', () => {
    it('should return company data via API', async () => {
      mockFetch.mockResolvedValue({
        ...
        json: async () => ({ success: true, total: 1, count: 1, data: [
          { id: '38460545', company: 'TALENT MATCHMAKERS S.R.L.', status: 'activ' }
        ] })
      });

      const result = await api.getCompanyByCif('38460545');

      expect(result).toHaveProperty('id', '38460545');
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(api.getCompanyByCif('38460545')).rejects.toThrow('API company search error: 401');
    });
  });
  ...
  describe('searchCompanyByName', () => {
    it('should return matching companies via API', async () => {
      ...
      const results = await api.searchCompanyByName('talent');

      expect(results).toHaveLength(1);
      expect(results[0].company).toContain('TALENT');
    });
  ...
  describe('upsertCompany', () => {
    it('should upsert company via API', async () => {
      mockFetch.mockResolvedValue({
        ok: true,
        json: async () => ({ success: true, id: '38460545', message: 'upserted' })
      });
    ...
    it('should throw on API error', async () => {
      ...
      await expect(api.upsertCompany({ id: '38460545', company: 'TEST' })).rejects.toThrow('API company upsert error: 500');
    });
  });
  ...
  describe('deleteJobsByCIF', () => {
    it('should delete all jobs for a CIF via API', async () => {
      ...
      await expect(api.deleteJobsByCIF('38460545')).resolves.not.toThrow();

      expect(mockFetch).toHaveBeenCalledWith(
        'https://api.peviitor.ro/v1/scraper/jobs/delete/',
        expect.objectContaining({
          method: 'DELETE',
          body: JSON.stringify({ cif: '38460545' })
        })
      );
    });
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(api.deleteJobsByCIF('38460545')).rejects.toThrow('API jobs delete error: 500');
    });
  });
  ...
  describe('Data Integrity', () => {
    it('should not have duplicate URLs for same CIF', async () => {
      mockFetch.mockResolvedValue(makeApiJobsResponse(2, [
        { url: 'https://test.com/job1', title: 'Job 1', cif: '38460545' },
        { url: 'https://test.com/job2', title: 'Job 2', cif: '38460545' }
      ]));
    ...
    it('should have valid CIF format for all jobs', async () => {
      mockFetch.mockResolvedValue(makeApiJobsResponse(2, [
        { url: 'https://test.com/1', title: 'Job 1', cif: '38460545' },
    ...
    it('should have valid status values', async () => {
      ...
      mockFetch.mockResolvedValue(makeApiJobsResponse(3, [
        { url: 'https://test.com/1', title: 'Job 1', cif: '38460545', status: 'scraped' },
        { url: 'https://test.com/2', title: 'Job 2', cif: '38460545', status: 'verified' },
        { url: 'https://test.com/3', title: 'Job 3', cif: '38460545', status: 'published' }
      ]));
